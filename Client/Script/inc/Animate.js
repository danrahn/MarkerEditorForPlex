...
const Animation = new function() {
    ...
    this.fireNow = function(func, element, ...args) {
        ...
        if (queue) {
            /*@__PURE__*/Log.tmi(
                queue,
                `FireNow - queue not empty, attempting to cancel current animations for ${element.id}`,
                true /*freeze*/);
     ...
     */
    const AnimationParams = function(func, prop, delay, ...args) {
        this.func = func;
        this.prop = prop;
        this.delay = delay;
        this.args = args;
     ...
     */
    const fireNext = function(element, removeElement=false) {
    ...
    const getStyle = (element) => getComputedStyle(element);
     ...
     */
    const animateColor = (element, prop, newColor, duration, deleteAfterTransition = false) => {
        // '(x + .5) | 0' == Math.round.
        // 'y || 1' because we need at least one step
        const steps = (duration / (50 / 3) + 0.5) | 0 || 1; // 1000 / 60 -> 60Hz
        const oldColor = new Color(getStyle(element)[prop]);
